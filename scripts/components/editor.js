...
export default function (data) {
  ...
  const handleButtonsSelected = (e) => {
    ...
    const currentSelected =
      currentTarget.parentElement.querySelector(".selectedButton");
  ...
  const handleAlignment = (e, alignTitle) => {
    const alignment = document.querySelector(
      ".mixtape__content > .metrics__container"
    );
  ...
  const downloadImage = () => {
    modernScreenshot
      .domToPng(document.querySelector(".mixtape"), {
        scale: 3,
      })
      .then((dataUrl) => {
        // No al then, si al async await
  ...
  buttonsTimes.forEach(({ name, events, id, base }) => {
    salem.createNode({
      ...
      attributes: [
        {
          attr: "class",
          value: `times__buttons--button ${
            base === true ? "selectedButton" : ""
          }`,
        },
        {
          attr: "id",
